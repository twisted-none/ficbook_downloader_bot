...
- принимает ссылки Ficbook, Wattpad и Hogwartsnet
- временно не принимает AO3 и Litnet из-за защитных проверок этих сайтов
...
- `AO3_LOGIN`, `AO3_PASSWORD` — зарезервированы до восстановления поддержки AO3
...
- `LITNET_LOGIN`, `LITNET_PASSWORD` — зарезервированы до восстановления поддержки Litnet
...
AO3 и Litnet временно отмечены как недоступные. Бот не обходит CAPTCHA, оплату или ограничения доступа. Если Ficbook временно ограничивает запросы, выполняются повторные попытки с backoff, после чего задание переносится в очередь следующего аккаунта.
